...
public class Member {
    ...
    private Guest guest;
    private String tier; // Silver, Gold, Platinum, Diamond, Elite
    private int points;
    ...
    public static Member registerNewMember(Guest guest) {
        if (guest == null) {
            return null;
        }
        
        Member member = new Member();
        member.memberId = "M" + System.currentTimeMillis();
        member.guest = guest;
        member.points = 100;
        member.lifetimePoints = 100;
    ...
    public boolean upgradeTier() {
        ...
        int maxIndex = getTierIndex("Elite");
    ...
    public boolean processRedemption(String item, int pointsRequired) {
        if (redeemPoints(pointsRequired)) {
            String redemptionRecord = String.format("%s (%d points) on %s - Status: COMPLETED", 
                item, pointsRequired, LocalDate.now().format(dateFormatter));
    ...
    public boolean requestRedemption(String item, int pointsRequired) {
        if (points >= pointsRequired) {
            String requestRecord = String.format("%s (%d points) on %s - Status: PENDING", 
                item, pointsRequired, LocalDate.now().format(dateFormatter));
    ...
    public String getNextTier() {
        ...
        return "Maximum tier reached (Elite)";
    }
    ...
    public List<String> getPersonalizedPromotions() {
        ...
        switch (this.tier) {
            case "Elite":
                promotions.add("🎖️ ELITE EXCLUSIVE: 50% off all room upgrades");
                promotions.add("🎖️ ELITE EXCLUSIVE: Free airport transfer");
                promotions.add("🎖️ ELITE EXCLUSIVE: Complimentary spa access");
                promotions.add("🎖️ ELITE EXCLUSIVE: Priority check-in/check-out");
                break;
            case "Diamond":
        ...
        if (pointsExpiryDate.minusMonths(1).isBefore(LocalDate.now())) {
            promotions.add("⏰ Use your points before they expire on " + pointsExpiryDate.format(dateFormatter));
        }
    ...
    public String toString() {
        return String.format("Member ID: %s, Guest: %s, Tier: %s, Points: %d (%d lifetime), Stays: %d, Spent: $%.2f",
                memberId, guest != null ? guest.getGuestName() : "No Guest", tier, points, lifetimePoints, stayCount, totalSpent);
    }
}